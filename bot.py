...
def get_price_coingecko(symbol, retries=2):
    ...
    for attempt in range(retries + 1):
        try:
            ...
            # Rate limited - don't waste retries here, let the caller fall back to Binance
...
def get_price_binance(symbol, retries=2):
    for attempt in range(retries + 1):
        try:
            resp = requests.get(
            ...
            if resp.status_code != 200:
                print(f"Binance error on {symbol}: status {resp.status_code}")
                time.sleep(1)
                continue
            ...
            print("Binance Fetch Error:", symbol, e)
            time.sleep(1)
    return None, None
...
def get_price(symbol):
    ...
    # Fall back to Binance if CoinGecko failed or was rate limited
...
while True:
    try:
        ...
        for name, symbol in PAIRS.items():
            ...
            # Space out requests so two pairs never hit CoinGecko in the same second
            time.sleep(5)
    ...
    # Longer pause between full cycles - CoinGecko free tier can't handle 30s polling
    time.sleep(60)
